...
function getRandomColor() {
  ...
  } while (COLORS.length > 1 && idx === lastColorIndex)
  lastColorIndex = idx
  return COLORS[idx]
}
...
const DEFAULT_WORDS = ["BX3", "BOOS BOT BUILDER"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const pixelSteps = 2
  const minParticleSize = 2, maxParticleSize = 4
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.particleSize = Math.random() * (maxParticleSize - minParticleSize) + minParticleSize
